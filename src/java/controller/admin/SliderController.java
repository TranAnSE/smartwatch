...
public class SliderController extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if ("add".equals(action)) {
            request.getRequestDispatcher("/admin/slider/add.jsp").forward(request, response);
        } else if ("edit".equals(action)) {
            int id = Integer.parseInt(request.getParameter("id"));
            Slider slider = sliderDAO.getSliderById(id);
            request.setAttribute("slider", slider);
            request.getRequestDispatcher("/admin/slider/edit.jsp").forward(request, response);
        } else {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if ("add".equals(action)) {
            ...
            sliderDAO.addSlider(new Slider(0, name, description, thumbnail, status));
        } else if ("edit".equals(action)) {
            int id = Integer.parseInt(request.getParameter("id"));
            String name = request.getParameter("name");
            String description = request.getParameter("description");
            ...
            sliderDAO.updateSlider(new Slider(id, name, description, thumbnail, status));
